...
clean-docs: ## clean docs
	rm -fv docs/source/gte.rst
	rm -fv docs/source/gte*.rst
	rm -fv docs/source/modules.rst

lint-flake8: ## check style with flake8
	flake8 gte gte/tests --exit-zero --count --statistics
lint-black: ## check style with black
	black --check gte gte/tests || true
...
coverage: ## check code coverage quickly with the default Python
	coverage run --source gte -m pytest
	coverage report -m
	coverage html
	$(BROWSER) htmlcov/index.html

docs: clean-docs ## generate Sphinx HTML documentation, including API docs
	sphinx-apidoc -o docs/source/ gte
	$(MAKE) -C docs clean
	$(MAKE) -C docs html
...
version-check: ## display git versioning tags and the actual current version from versioneer
	@git tag | grep "gte-v" | sed -e "s|gte-v||g"
	@echo "----------------"
	@python -c "import gte; print(gte.__version__+' [← current]')"
